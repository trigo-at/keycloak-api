...
const {omit, path, mergeDeepRight, merge} = require('ramda');
...
async function parseReponse(response) {
	...
	if(response.headers.get('location')) {
		...
		newItemId = locationHeader.substring(locationHeader.lastIndexOf('/') + 1)
	}
	if (response.status === 204) {
		return {statusCode: 204, location: locationHeader, id: newItemId};
	}
	let data;
	if (
	...
	const res = {data, statusCode: response.status, location: locationHeader, id: newItemId};
	return res;
}
...
async function fetchAPI(opts) {
	...
	const response = await fetchWithRetry(
		urlStr,
		options,
		path(['options', 'maxRetries'], opts || 2)
	);
