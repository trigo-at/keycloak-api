...
class KeycloakApi {
	...
	async getHeaders({ tokenProvider = null }) {
		if (!tokenProvider) throw new Error('Mandatory argument "tokenProvider" missing');

		const token = tokenProvider ? await tokenProvider() : (await this.getAdminApiAuthToken()).data.access_token;
		return {
	...
	async createRealm({ realm, tokenProvider }) {
		const headers = await this.getHeaders({ tokenProvider });
