...
const getToken = async ({ tokenProvider, ctx }) => {
	...
	if (tokenProvider.__currentToken) {
		...
		ctx.log.info({ inAMinute, expiresAt: tokenProvider.__currentToken.expiresAt });
	}
	if (tokenProvider.__currentToken
		&& (inAMinute <= tokenProvider.__currentToken.expiresAt || !tokenProvider.__currentToken.expiresAt)) {
	...
	if (tokenProvider.__currentToken) {
		ctx.log.info('Found expiring token:', tokenProvider.__currentToken.expiresAt);
	}
	...
	throw Boom.internal('Unable to fetch a access tokent with the given tokenProvider');
};
