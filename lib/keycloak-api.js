...
const debug = require('debug')('@trigo/keycloak-api');
...
class KeycloakApi {
	...
	async getGroupMembers({realm, groupId, first, max, tokenProvider}) {
		return retryOn401({
			fn: requests.getGroupMembers,
			options: {realm, groupId, first, max},
			getHeaders,
			tokenProvider,
			ctx: this,
		});
	...
	async getEvents({realm, query, tokenProvider}) {
		return retryOn401({
			fn: requests.getEvents,
			options: {realm, query},
			getHeaders,
			tokenProvider,
			ctx: this,
	...
	async getAdminEvents({realm, query, tokenProvider}) {
		return retryOn401({
			fn: requests.getAdminEvents,
			options: {realm, query},
			getHeaders,
			tokenProvider,
			ctx: this,
	...
	async getComponents({realm, query, tokenProvider}) {
		return retryOn401({
			fn: requests.getComponents,
			options: {realm, query},
			getHeaders,
			tokenProvider,
			ctx: this,
	...
	async createComponent({realm, component, mapper, tokenProvider}) {
		return retryOn401({
			fn: requests.createComponent,
			options: {realm, component, mapper},
			getHeaders,
			tokenProvider,
			ctx: this,
		});
